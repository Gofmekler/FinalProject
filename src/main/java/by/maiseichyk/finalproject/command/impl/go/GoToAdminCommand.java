...
import static by.maiseichyk.finalproject.command.PagePath.ADMINS_LIST;
...
public class GoToAdminCommand implements Command {
    ...
    public Router execute(HttpServletRequest request) throws CommandException {
        ...
        try {
            List<User> users = userService.findUsers(UserRole.ADMIN.toString());
            if (users.isEmpty()) {
                request.setAttribute(RequestAttribute.USERS_LIST_MESSAGE, NO_USERS_MESSAGE);
            } else {
                session.setAttribute(SessionAttribute.ADMINS, users);
            }
        } catch (ServiceException e) {
            LOGGER.error("Exception while searching admins." + e);
            throw new CommandException("Exception while searching admins.", e);
        }
        return new Router(ADMINS_LIST, FORWARD);
    }
}
